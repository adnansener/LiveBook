<?php
    ...
    if(isset($_POST['change'])){
        ...
        if(mysqli_num_rows($result)===1){
            ...
            if($row['user_name'] != $new_value){
                $_SESSION['user_name'] = $new_value;

                $old_value = $row['user_name'];
                
                $sql = "UPDATE user_info SET user_name = '".$new_value."' WHERE user_id = $id";
                mysqli_query($connect, $sql);

                $sql = "UPDATE user_thoughts SET user_name = '".$new_value."' WHERE user_name = '$old_value'";
                mysqli_query($connect, $sql);
                ...
                ECHO '<script>alert("Your new username cannot be the same as your old one.")</script>';
            }
        }
        else {
...
<html lang="tr">
    ...
    <body>
        ...
        <div class="container">
            <form action="registeration_username.php" method="post" class="form" role="form">
